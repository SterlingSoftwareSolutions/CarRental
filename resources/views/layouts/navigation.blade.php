<nav class="z-50 w-full nav">
	...
	<ul class="flex items-center justify-between w-2/6 p-4 mx-auto nav-links">
		<li><a class="text-white font-semibold hover:text-[#307449]" href="{{route('home')}}">HOME</a></li>
		<li><a class="text-white font-semibold hover:text-[#307449]" href="{{route('carlist')}}">VEHICLE LIST</a></li>
		<li><a class="text-white font-semibold hover:text-[#307449]" href="{{route('about')}}">ABOUT US</a></li>
		...
		<li>
			<div x-data="{dropdownMenu: false}" class="relative"> <!-- Dropdown toggle button --> <img src="@if(isset(Auth::user()->images[0])){{ Storage::url(Auth::user()->images[0]->file_path) }}@else
				...
				<div x-show="dropdownMenu"
					class="absolute right-0 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 md:bottom-auto bottom-12">
